 ...
 * Supports both Ollama-based chat and simulated chat responses
 */
  ...
  async getAIResponse(documentId, documentText, userMessage) {
    ...
    if (this.useAI) {
      try {
        ...
        if (connectionTest) {
          // Create a system prompt that includes the document text
...
Keep responses concise and professional.`;
          ...
          console.log('Used Ollama for chat response');
        } else {
          // Fall back to mock response
          response = this.generateMockResponse(userMessage, documentText);
